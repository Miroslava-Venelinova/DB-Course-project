...
CREATE OR ALTER PROC usp_DeleteMajorFee @major_id INT
AS
	DELETE FROM Majors_Fees
	WHERE major = @major_id
GO
...
CREATE OR ALTER PROC usp_DeleteTeacherSubjectPair @teacher_id INT, @subject_id INT
AS
	DELETE FROM Teachers_Subjects
...
CREATE OR ALTER PROC usp_DeleteStudentSubjectPair @student_id VARCHAR(10), @subject_id INT
AS
	DELETE FROM Students_Subjects
...
CREATE OR ALTER PROC usp_UpdateStudentStream @faculty_number VARCHAR(10), @stream INT
AS
	UPDATE Students
	SET [stream] = @stream
	WHERE faculty_number = @faculty_number
